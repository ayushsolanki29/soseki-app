...
export default function PaymentsPage() {
  ...
  payments.forEach(payment => {
    ...
    if (lastGroup && lastGroup.date === dateStr) {
      ...
      groupedPayments.push({ date: dateStr, items: [payment] });
    }
  });
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              groupedPayments.flatMap((group) => [
                <TableRow key={`header-${group.date}`} className="bg-muted/50 hover:bg-muted/50">
                  <TableCell colSpan={6} className="font-semibold text-xs text-muted-foreground uppercase tracking-wider py-2">
                    {group.date}
                  </TableCell>
                </TableRow>,
